...
export default function HouseDreams() {
  ...
  const submitDream = trpc.dreams.submit.useMutation({ onSuccess: () => { setDreamSubmitted(true); setDreamText(""); } });
  ...
  return (
    <div className="dream-page">
      ...
      <main>
        ...
        <section className="dream-console"><div className="dream-receive"><p className="eyebrow oxblood">The Dream Door receives</p><h2>Describe what<br /><em>you cannot describe.</em></h2><p>Leave a fear, memory, or impossible room. The house will hold the words as a provisional chamber.</p><textarea aria-label="Dream submission" value={dreamText} onChange={(event) => setDreamText(event.target.value)} placeholder="I remember a room where…" /><button type="button" disabled={submitDream.isPending || !dreamText.trim()} onClick={() => submitDream.mutate({ title: dreamName.trim() || dream.title, dreamText })}>{submitDream.isPending ? "Listening…" : "Submit dream"} <ArrowUpRight size={15} /></button>{dreamSubmitted && <p className="dream-inscription">The house has received your dream.</p>}{submitDream.error && <p className="form-confirmation">The door is closed for a moment. Try again.</p>}</div><div className="dream-console-main"><div className="dream-console-head"><p className="eyebrow oxblood">A generative field note</p><h2>Give the dream<br /><em>a name.</em></h2><p>Every chamber is a temporary record. Name what you saw and the house will hold it for this session.</p></div><form className="dream-name-form" onSubmit={(event) => { event.preventDefault(); if (dreamName.trim()) setInscribedName(dreamName.trim()); }}><label htmlFor="dream-name">Your title for this dream</label><div><input id="dream-name" value={dreamName} onChange={(event) => setDreamName(event.target.value)} placeholder="Name the thing behind the door" /><button type="submit">Inscribe <ArrowUpRight size={16} /></button></div></form>{inscribedName && <p className="dream-inscription">Dream {dream.id} is now filed as <em>{inscribedName}</em>.</p>}</div><aside className="dream-specs"><span>MATERIAL</span><strong>{dream.material}</strong><span>SOUND</span><strong>{dream.sound}</strong><span>FRAGMENT</span><button type="button" onClick={() => setFragmentIndex((current) => (current + 1) % fragments.length)}>{fragment} <ArrowRight size={14} /></button></aside></section>
        <section className="dream-recent"><p className="eyebrow oxblood">Dreams from other visitors</p><h2>The lucid archive</h2>{recentDreams.data?.length ? <div>{recentDreams.data.slice(0, 4).map((entry) => <article key={entry.id}><span>{new Date(entry.createdAt).toLocaleDateString()}</span><h3>{entry.title}</h3><p>{entry.dreamText}</p></article>)}</div> : <p>The first chamber is waiting for a witness.</p>}</section>
