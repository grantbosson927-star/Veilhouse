...
export default function HouseDreams() {
  ...
  const generatedDreams = trpc.dreams.mine.useQuery(undefined, { enabled: Boolean(user), retry: false });
  ...
  const generateDream = trpc.dreams.generate.useMutation({ onSuccess: (result) => { setGeneratedImage(result.dream?.imageUrl || ""); void utils.account.snapshot.invalidate(); void utils.dreams.mine.invalidate(); } });
  ...
  return <div className={roomTurned ? "dream-page is-turned" : "dream-page"}>
    <header className="dream-topbar"><a className="wordmark" href="/"><span>VEIL</span><i>HOUSE</i></a><a className="detail-back" href="/#signal"><ArrowLeft size={15} /> Exit chamber</a><span className="detail-issue">DREAM / {dream.id}</span><AccountRitual compact /></header>
    <main>
