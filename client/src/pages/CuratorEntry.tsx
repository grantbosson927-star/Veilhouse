...
export default function CuratorEntry() {
  ...
  const signIn = trpc.curator.signIn.useMutation({
    onSuccess: async (result) => {
      if (result.unlocked) {
        await refresh();
        navigate("/curator-admin");
      } else {
        setMessage("The house has recorded your address. If you are the curator, use the authorized account to open the private desk.");
      }
    },
  });
  const [email, setEmail] = useState("");
  const [message, setMessage] = useState("");
  const busy = unlock.isPending || signIn.isPending;
  ...
  const submit = (event: FormEvent) => {
    ...
    signIn.mutate({ email: normalized });
  };
  ...
  return (
    <div className="admin-gate">
      ...
      <p>Visitors may leave an email for the archive. The project curator can enter that address to open the private desk and upload new matter to Cloudflare R2.</p>
      ...
      {!user && <a className="button-outline" href="/enter">Visitor sign in / create account <ArrowUpRight size={16} /></a>}
