  ...
  return (
    <div className="site-shell">
      ...
      <main id="top">
        ...
        <section className="signal" id="signal"><div className="signal-copy"><p className="eyebrow oxblood">A note from the other side</p><h2>Stay close<br /><em>to the signal.</em></h2><p>Receive one dispatch every other Thursday.</p><form className="signup-form" onSubmit={(event) => { event.preventDefault(); if (email.trim()) setSubscribed(true); }}><label htmlFor="email">Receive one dispatch every other Thursday.</label><div><input id="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="your@email.com" required /><button type="submit" aria-label="Subscribe"><ArrowUpRight size={16} /></button></div></form>{subscribed && <p className="form-confirmation">The signal has your address.</p>}<div className="house-dream"><p className="eyebrow">The house dreams</p><p className="dream-line">A room that changes when you name it.</p><a className="button-outline" href="/dreams">Enter Dream 00012 <ArrowUpRight size={16} /></a></div></div><div className="calendar-panel"><p className="eyebrow oxblood">The house, in rhythm</p><h3>One disturbance<br /><em>every day.</em></h3><div>{calendar.map(([day, title]) => <span key={day}><b>{day}</b>{title}</span>)}</div></div></section>
      </main>
