...
class SDKServer {
  ...
  async authenticateRequest(req: { headers: { cookie?: string | string[]; authorization?: string | string[] } }): Promise<AuthenticatedUser> {
    ...
    if (!user && sessionUserId.startsWith("curator:")) {
      const email = sessionUserId.slice("curator:".length);
      return {
        id: 1,
        openId: sessionUserId,
        name: session.name || "Curator",
        email,
        passwordHash: null,
